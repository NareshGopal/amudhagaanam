...
Songs.prototype.addSongToPlaylist = function (playlistId, songId, callback) {
  var _this = this;

  var playlist = this.getPlaylist(playlistId);
  if (Object.keys(playlist).indexOf("songs") != -1) {
  ...
  var pathname = path.join(_this.dataPath, "playlists", playlistId + ".json");
  fs.writeFileSync(pathname, JSON.stringify(playlist));

  callback(null, playlistId);
};

Songs.prototype.deleteSongFromPlaylist = function (
  playlistId,
  songId,
  callback
) {
  ...
  var playlist = this.getPlaylist(playlistId);
  var spliceIndex = playlist.songs.indexOf(songId);
  playlist.songs.splice(spliceIndex, 1);
